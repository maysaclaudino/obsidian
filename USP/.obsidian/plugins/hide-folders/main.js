...
var HideFoldersPlugin = class extends import_obsidian.Plugin {
  async processFolders() {
    this.settings.attachmentFolderNames.forEach((folderName) => {
      const folderElements = document.querySelectorAll(`[data-path$="/${folderName.trim()}"${this.settings.matchCaseInsensitive ? " i" : ""}], [data-path="${folderName.trim()}"${this.settings.matchCaseInsensitive ? " i" : ""}]`);
...
var HideFoldersPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Folders to hide").setDesc("The name of the folders to hide, seperated by new lines").addTextArea((text) => text.setPlaceholder("attachments").setValue(this.plugin.settings.attachmentFolderNames.join("\n")).onChange(async (value) => {
